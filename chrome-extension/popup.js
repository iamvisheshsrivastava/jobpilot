...
// silent=true: just grabs text, doesn't touch the form UI (used by Fit/Skills)
function autoCaptureAndRun(callback, silent) {
  if (capturedPageText) { callback(capturedPageText); return }

  if (!silent) showMsg(saveStatus, 'Reading page…', 'processing')
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    const tab = tabs?.[0]
...
suitBtn.addEventListener('click', () => {
  ...
  }, true) // silent — don't show captured fields
})
